...
**Progress: 428 / 705 programmes translated** (top 428 by rank). Scholarships: **0 / 197**. The mechanism is fully built and proven (see the "Catalogue i18n engine" section below). To continue, repeat this loop:
...
2. **Catalogue i18n engine — programme/scholarship DB text is now translatable per language.** The catalogue is fetched from Supabase at runtime, so the build-time dictionary can't reach it. New mechanism:
   ...
   - **Progress:** programmes **428 / 705** translated (top 428 by rank, batches 1–36 this session). Scholarships **0 / 197**. **This is the big remaining grind** — ~900 free-text rows × 4 langs. Keep going by rank/visibility order (`select id, program, blurb, highlights from public.programmes where i18n is null order by rank limit 12`). The user explicitly wants the whole catalogue translated ("translate everything", "don't stop").
